...
export default function MockGoalBoardScreen(){
  return <PageShell><PageHeader title="Goal Board" eyebrow="MOCK DATA V3" back/><MockDataBanner/><SectionLabel title="30 GOALS" meta="ACTIVE + COMPLETED"/><MockList items={mockGoals.slice(0,18).map(g=>({id:g.id,title:g.title,meta:`${g.category} · due ${g.due}`,value:`${Math.round(g.progress*100)}%`}))}/><View style={s.gap}/><Surface><Text style={s.title}>Next actions</Text>{mockGoals.slice(0,5).map(g=><Text key={g.id} style={s.copy}>• {g.nextAction}</Text>)}</Surface></PageShell>;
}
