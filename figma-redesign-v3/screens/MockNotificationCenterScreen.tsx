...
export default function MockNotificationCenterScreen(){
  return <PageShell><PageHeader title="Notification Center" eyebrow="MOCK DATA V3" back/><MockDataBanner/><SectionLabel title="36 NOTIFICATIONS" meta="MIXED READ STATE"/><MockList items={mockNotifications.map(n=>({id:n.id,title:n.title,meta:`${n.type} · ${n.timestamp}`,value:n.unread?'NEW':''}))}/><View style={s.gap}/><Surface><Text style={s.title}>Read-state coverage</Text><Text style={s.copy}>Every fourth fixture is unread so badges, dots and grouped notification headers have data.</Text></Surface></PageShell>;
}
