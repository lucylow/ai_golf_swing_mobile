...
import { mockPhases } from '../mock';
export default function MockPhaseLibraryScreen(){
  return <PageShell><PageHeader title="Phase Library" eyebrow="MOCK DATA V3" back/><MockDataBanner/><SectionLabel title="7 SWING PHASES" meta="MODEL FIXTURES"/><MockList items={mockPhases.map(p=>({id:p.id,title:p.label,meta:`${p.score} score · ${p.durationMs} ms · ${p.finding}`,value:`${p.minDegrees}–${p.maxDegrees}°`}))}/><View style={s.gap}/>{mockPhases.slice(0,4).map(p=><Surface key={p.id}><Text style={s.title}>{p.label}</Text><Text style={s.copy}>{p.cue}</Text><Text style={s.copy}>{p.checkpoints.join(' · ')}</Text></Surface>)}</PageShell>;
}
