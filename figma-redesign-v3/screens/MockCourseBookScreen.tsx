...
export default function MockCourseBookScreen(){
  return <PageShell><PageHeader title="Course Book" eyebrow="MOCK DATA V3" back/><MockDataBanner/><SectionLabel title="COURSE HISTORY" meta="24 FIXTURES"/><MockList items={mockCourses.map(c=>({id:c.id,title:c.name,meta:`${c.city} · ${c.holes} holes · slope ${c.slope}`,value:String(c.score)}))}/><View style={s.gap}/><Surface><Text style={s.title}>Round context</Text><Text style={s.copy}>Course fixtures provide recap, score-summary and history-filter content while remaining fictional.</Text></Surface></PageShell>;
}
