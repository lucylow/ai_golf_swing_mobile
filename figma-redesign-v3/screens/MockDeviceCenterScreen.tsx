...
export default function MockDeviceCenterScreen(){
  return <PageShell><PageHeader title="Device Center" eyebrow="MOCK DATA V3" back/><MockDataBanner/><SectionLabel title="CONNECTED HARDWARE" meta="18 FIXTURES"/><MockList items={mockDevices.map(d=>({id:d.id,title:d.name,meta:`${d.type} · ${d.firmware} · ${d.lastSeen}`,value:d.connected?`${d.battery}%`:'OFF'}))}/><View style={s.gap}/><Surface><Text style={s.title}>Capture stack</Text><Text style={s.copy}>Connected/unconnected and high/low battery states are available without touching real hardware.</Text></Surface></PageShell>;
}
