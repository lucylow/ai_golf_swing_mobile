...
export function getSessionCookieOptions(
  ...
  const headerHostname = Array.isArray(forwardedHost) ? forwardedHost[0] : forwardedHost?.split(",")[0]?.trim();
  const hostname = req.hostname || headerHostname?.split(":")[0] || "localhost";
